...
exports.markRoomRead = async (req, res) => {
    ...
    const r = await pgPool.query(`SELECT msg_seq FROM Room_Counters WHERE room_id = $1`, [room_id]);
    if (r.rowCount === 0) return res.status(404).json({ message: 'Room not found' });
    const seq = r.rows[0].msg_seq;
    console.log(`readController: markRoomRead: user_id: ${user_id}, room_id: ${room_id}, seq: ${seq}`);
...
exports.listUnreads = async (req, res) => {
    const user_id = req.user.id;
    ...
    const data = rows.rows.map(r => ({ room_id: r.room_id, unread: Number(r.unread) })); // ensure unread is a number
